...
*/
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////
  // START CODE FOR ADS MANAGEMENT
  ///////////////////////////////////////////////////////////////////////////////////////////////

  public function adsIndex()
  {
    return view('admin.ads_space');
  }
  ...
  public function adsStore(Request $request)
  {
    $data = $request->validate([
      ...
      'link' => 'required|url',
      'position' => 'required|in:topbar,sidebar', // Added position validation
    ]);
  ...
  {
    Log::info('Incoming Request: ', $request->all()); // Full request data
    $request->validate([
      'title' => 'required|string|max:255',
      'description' => 'nullable|string',
      'link' => 'required|url',
      'image' => 'nullable|image|mimes:jpg,jpeg,png,gif|max:2048',
      'position' => 'required|in:topbar,sidebar',
    ]);
    ...
    if ($request->hasFile('image')) {
      Log::info('File Detected: ' . $request->file('image')->getClientOriginalName());
      if ($ad->image && Storage::disk('public')->exists($ad->image)) {
        Storage::disk('public')->delete($ad->image);
        Log::info('Old Image Deleted: ' . $ad->image);
      }
      $path = $request->file('image')->store('ads', 'public');
      $ad->image = $path;
      Log::info('New Image Path Saved: ' . $path);
    } else {
      Log::info('No new image file detected in request');
    }

    $ad->save();
    Log::info('Ad Saved with Image: ' . $ad->image);

    return response()->json(['success' => true, 'message' => 'Ad updated successfully!', 'new_image' => $ad->image]);
  }
    ...
    if ($request->ajax()) {
      ...
      return datatables()->of($ads)
        ...
        ->addColumn('image', function ($row) {
          $imagePath = $row->image ? asset('storage/' . $row->image) : '';
          Log::info('Rendering Image Path for ID ' . $row->id . ': ' . $imagePath); // Debugging
          return $row->image ? '<img src="' . asset('storage/' . $row->image) . '" width="80" alt="Ad Image">' : 'No image';
        })
        ->addColumn('link', function ($row) {
          return '<a href="' . $row->link . '" target="_blank">' . $row->link . '</a>';
        })
        ->addColumn('position', function ($row) {
          return $row->position ?? 'Not set';
        })
        ->addColumn('action', function ($row) {
          return '
                    <button class="btn btn-sm btn-info viewAdBtn" data-id="' . $row->id . '">View</button>
                    <button class="btn btn-sm btn-warning editAdBtn" data-id="' . $row->id . '">Edit</button>
                    <button class="btn btn-sm btn-danger delAdBtn" data-id="' . $row->id . '">Delete</button>
                ';
        })
  ...
  {
    return view('admin.ads_space', ['mode' => 'create']);
  }

  public function adsShow(Ad $ad)
  {
    return response()->json([
      'id' => $ad->id,
      'title' => $ad->title,
