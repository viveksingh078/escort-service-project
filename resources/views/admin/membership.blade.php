        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content rounded-0">
                <div class="modal-body">
                    <form id="planForm" method="POST" action="{{ route('admin.membership.store') }}">
                        @csrf
        ...
        jQuery(document).ready(function ($) {
            ...
            function loadPlans() {
                $('#subscriptionTable').DataTable({
                    ...
                    destroy: true,
                    ajax: {
                        url: '{{ route("admin.membership.list") }}',
                        type: 'GET'
                    },
                    columns: [
            ...
            function submitPlanForm(id, planName, planPrice, planDuration, planFeature) {
                const url = id ? '{{ route("admin.membership.update", ["id" => ":id"]) }}'.replace(':id', id) : '{{ route("admin.membership.store") }}';
            ...
            $(document).on('click', '.editBtn', function () {
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route("admin.membership.edit", ["id" => ":id"]) }}'.replace(':id', id), // Route fix
                    type: 'GET',
            ...
            $(document).on('click', '.delBtn', function () {
                ...
                if (confirm('Are you sure you want to delete this plan?')) {
                    $.ajax({
                        url: '{{ route("admin.membership.delete", ["id" => ":id"]) }}'.replace(':id', id),
                        type: 'DELETE',
