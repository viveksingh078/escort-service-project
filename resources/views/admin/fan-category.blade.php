...
<div class="container-fluid p-0 m-0 py-4">
  <div class="container py-5 px-5 bg-white">
    ...
    <div class="table-responsive">
      <table id="fanCategoryTable" class="table table-bordered">
        <thead>
          <tr>
            <th>#</th>
            <th>Category Name</th>
...
jQuery(document).ready(function ($) {
  ...
  // Load fan Categories Table
  function loadfanCategories() {
    $('#fanCategoryTable').DataTable({
      processing: true,
      serverSide: true,
      destroy: true,
      ajax: {
        url: '{{ route("admin.fan.categories.list") }}',
        type: 'GET'
      },
      columns: [
  ...
  $('#add-category-form').on('submit', function (e) {
    ...
    $.ajax({
      url: "{{ url('/admin/add-fan-category') }}",
      type: "POST",
      data: {
        _token: '{{ csrf_token() }}',
        category_name,
  ...
  $(document).on('click', '.editBtn', function () {
    ...
    $.get('/admin/edit-fan-category/' + id, function (data) {
      $('#edit_id').val(data.id);
  ...
  $('#edit-category-form').on('submit', function (e) {
    ...
    $.ajax({
      url: '/admin/update-fan-category/' + id,
      method: 'PUT',
      data: {
        _token: '{{ csrf_token() }}',
        category_name,
  ...
  $(document).on('click', '.delBtn', function () {
    ...
    if (confirm('Are you sure you want to delete this category?')) {
      $.ajax({
        url: '/admin/delete-fan-category/' + id,
        type: 'DELETE',
